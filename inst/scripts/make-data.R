### =========================================================================
### allowranges is an AnnotationHub package that stores genomic coordinates of
### DNAse I hypersensitive regions (open/active chromatin) as GRanges. 
### -------------------------------------------------------------------------
###

# allowranges package currently contains 16 Rds objects with cell/tissue-specific GRanges.
...
library(ggplot2)
# Folder with results
dir_in <- "/Volumes/GoogleDrive/My Drive/allowdata"
...
chrom_data <- chrom_data[chrom_data$chrom %in% seqlevels(allowGR), ]
chrom_data <- chrom_data[match(seqlevels(allowGR), chrom_data$chrom), ]
...
for (cell_type in cell_types) {
  ...
  allowGR <- GenomicRanges::makeGRangesFromDataFrame(mtx_subset, keep.extra.columns = TRUE)
  # Check if chromosome order is the same
  if (!all.equal(seqlevels(allowGR), chrom_data$chrom)) {
    print(paste("Chromosome order does not match for", genome_id, "genome."))
    break
  }
  # Assign seqinfo data
  seqlengths(allowGR) <- chrom_data$size
  isCircular(allowGR) <- chrom_data$circular
  genome(allowGR)     <- genome_id
  ...
  # Save as Rds object
  saveRDS(object = allowGR, file = file.path(dir_in, fileout))
}
